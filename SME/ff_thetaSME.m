            ...
            for subind = 1:length(subjects_total)
                ...
                for base_ind = 1:length(base_ind_total)
                    for ROI_ind = 1:length(ROI_ind_total)
                        ...
                        for i = 1:length(f1)
                            for j = 1:length(f2)
                                band = [f1(i)-df, f1(i)+df; f2(j)-f1(i),f2(j)+f1(i)];
                                gammaband = band(2,:);
                                thetaband = band(1,:);
                                amp_gamma = cell(1,2);
...
%                                 
%                                 if pvalue1<0.05
%                                     phase_su{sigacc} = circ_mean(pfPha{1});
%                                     phase_unsu{sigacc} = circ_mean(pfPha{2});
%                                     sig_f{sigacc} = f1(i);
%                                     whatRcomputing2 = string(strcat(base,'_',ROI,hemi));
%                                     sig_thiscon{sigacc} = whatRcomputing2;
%                                     MIs{sigacc} = mean([M_norm{1}]);
%                                     MIuns{sigacc} =mean([M_norm{2}]);
%                                     sigacc= sigacc+1;
%                                 end
...
%                                 PLV_unsu{eleacc}{i} = PLVs{2};
                        ...
                        eleacc = eleacc+1;
                        toc
%                         sprintf(strcat('No.ele=', num2str(eleacc)));
%                         sprintf(strcat('No.sig=', num2str(sigacc)));
%                         if sigacc >1
%                             sig_MI = [sig_f;sig_thiscon;MIs;MIuns];
%                             sig_phaamp = [phase_su;phase_unsu];
%                             % save 2
%                             path2save = string(strcat(cd,'/ff/Sig_pairs/'));
%                             if ~exist(path2save,'dir')
%                                 mkdir(path2save)
%                             end
%                             file2savesig = strcat(path2save, ROI,'_MI.mat');
%                             file2savesig2 = strcat(path2save, ROI,'_phaamp.mat');
%                             save(string(file2savesig),'sig_MI')
%                             save(string(file2savesig2),'sig_phaamp')
%                         end
...
%                         end
                   ...
                   %     MI_total = [MI_su;MI_unsu];
                    ...
                    %    PLV_total = [PLV_su;PLV_unsu];
                        ...
                        save(string(file2save2),'SME_total');
            ...
            clear  MI_su MI_unsu SME_su SME_unsu PLV_su PLV_unsu
